...
# Configuration - set repository and target subdirectory for backend review
TOKEN = os.getenv("GITHUB_TOKEN")
OWNER = "kevin2-cyber"
REPO = "oracle-java-foundations"
BRANCH = os.getenv("BRANCH", "master")
TARGET_SUBDIR = "el-apunte-grpc/"

ALLOWED_EXTENSIONS = {'.java', '.kt', '.proto', '.xml', '.gradle', '.md'}
...
print(f"Analyzing /{OWNER}/{REPO} file structure for '{TARGET_SUBDIR}'...")
...
output_filename = f"{REPO}_el_apunte_grpc_backend_codebase.txt"
with open(output_filename, "w", encoding="utf-8") as outfile:
    outfile.write(f"# BACKEND SYSTEM CONTEXT: {OWNER}/{REPO} ({TARGET_SUBDIR})\n\n")
    outfile.write("## Module and Package Architecture Map\n```text\n")
    for item in tree_data:
        path = item['path']
        if not any(path.startswith(p) for p in EXCLUDED_PATHS) and path.startswith(TARGET_SUBDIR):
    ...
    for item in tree_data:
        if item.get("type") == "blob":
            file_path = item["path"]
            if not file_path.startswith(TARGET_SUBDIR):
                continue
...
print(f"\nSuccess! Final backend context payload compiled to '{output_filename}'.")
